...
public class ScientificExperiment : MonoBehaviour
{
    ...
    {
        if (!this.mXMLLoaded || UICursorManager.pCursorManager == null || this._MainUI == null || SanctuaryData.pInstance == null)
        {
            return;
        }
        ...
        else
        {
            ...
            if (this.mExperiment == null && this.mKAUIGenericDB == null)
            {
                ...
                if (this.mKAUIGenericDB != null)
                {
                    ...
                    KAUI.SetExclusive(this.mKAUIGenericDB, false);
                }
            }
        }
        if (this.mCurrentDragon == null)
        {
            if (!this.mCreatedDragon && this.mExperiment != null)
            {
                if (this.mExperiment.Type == 1)
                {
                    this.InitGronckleExp();
                }
                else
                {
                    this.CreateDragon(this.mExperiment.DragonType, this.mExperiment.DragonStage, this.mExperiment.DragonGender);
                }
    ...
    public void InitGronckleExp()
    {
        ...
        if (sanctuaryPetTypeInfo._AgeData[ageIndex]._PetResList[0]._Gender == Gender.Male)
        {
        ...
        RaisedPetData pdata = RaisedPetData.CreateCustomizedPetData(this._GronckleId, RaisedPetStage.ADULT, resName, Gender.Male, null, true);
        SanctuaryPet component = this._Gronckle.GetComponent<SanctuaryPet>();
        component.Init(pdata, false, null);
    ...
    private void InitializeDragonData()
    {
        ...
        if (typeInfo != null && this.mCurrentDragon.pCurAgeData != null)
        {
            ...
            {
                string[] array = this.pDragonAgeData._BreathFireParticleRes.Split('/', StringSplitOptions.None);
                if (array.Length == 3)
                {
    ...
    public void OnExperimentTaskDone(LabTask inExpTask)
    {
        ...
        if (MissionManager.pInstance != null)
        {
            ...
            if (tasks != null)
            {
                ...
                {
                    task.pPayload.Set("Lab_Result", inExpTask.ResultText._Text);
                    task.pPayload.Set("Lab_Result_ID", inExpTask.ResultText._ID.ToString());
                }
            }
            ...
            if (this.pExperiment.AreAllTasksDone() && !string.IsNullOrEmpty(this.pExperiment.ResultImage) && mission != null)
            {
                ...
                if (MissionManagerDO.GetScientificQuestPhase(mission, RuleItemType.Mission, 2, out obj, out ruleItemType))
                {
                    Task task2 = obj as Task;
                    if (task2 != null && task2.Completed > 0)
                    {
    ...
    public void Exit()
    {
        ...
        RsResourceManager.LoadLevel(ScientificExperiment.mLastScene, false);
    ...
    public void StopDragonAnim()
    {
        ...
        if (this.mCurrentDragon != null)
        {
            this.OnAnimEvent(new AvAvatarAnimEvent
            {
                _Animation = this.mCurrentDragon.pAnimToPlay,
                mData = new AnimData(),
                mData =
                {
                    _DataString = "Finished"
                }
            });
    ...
    private void PlayParticle(GameObject inObj, bool inPlay)
    {
        ...
        if (component != null)
        {
            UtDebug.Log("Lab: Emitting particle " + inObj.name + " - " + inPlay.ToString(), 8U);
    ...
    private void OnAnimEvent(AvAvatarAnimEvent inEvent)
    {
        ...
        uint num = < PrivateImplementationDetails >.ComputeStringHash(dataString);
        if (num > 1306582585U)
        {
            if (num <= 2813513411U)
            {
                if (num != 2209866171U)
                {
                    ...
                    {
                        SECrucibleContextSensitive.pInstance.pUI.DisableWidget("Heat", false);
                        return;
                    }
                }
                else
            ...
            else
            {
                ...
                {
                    SECrucibleContextSensitive.pInstance.pUI.DisableWidget("Heat", true);
                }
                this.PlayParticle(this.mDragonFlameParticle, true);
                this.pCrucible.Heat();
                SnChannel.Play(this._DragonFireSFX, "Default_Pool3", true, null).pLoop = false;
                return;
            }
            return;
        }
        ...
        else
        {
            ...
            this.PlayParticle(this.mDragonFlameParticle, true);
            SnChannel.Play(this._SkrillBreatheSFX, "Default_Pool3", true, null).pLoop = false;
    ...
    private void MagnetTweenDone()
    {
        if (this.mCrucible.pTestItems != null)
        {
            ...
            for (int i = 0; i < this.mCrucible.pTestItems.Count; i++)
            {
                ...
                {
                    this.mCrucible.pTestItems[i].gameObject.GetComponent<PendulumEffect>().enabled = true;
                    flag = true;
                }
            }
            if (flag)
            {
                Animation component = this._MagnetGameObject.GetComponent<Animation>();
                component.Play("Attract");
                base.Invoke("OnAttractAnimEnd", component.clip.length / 2f);
                return;
            }
    ...
    private void OnAttractAnimEnd()
    {
        for (int i = 0; i < this.mCrucible.pTestItems.Count; i++)
        {
            ...
            if (this.IsMagneticObject(labTestObject))
            {
                ...
                component._AttractiveGameObj = this._MagnetAttachmentNode;
                component._OnAttractiveObjHit = new AttractableObject.TargetGameObjectHit(this.OnTargetObjectHit);
                component.enabled = true;
            }
        }
    }
    ...
    public void CreateDragon(int inPetTypeID, RaisedPetStage inStage, Gender inGender)
    {
        ...
        {
            SanctuaryManager.CreatePet(pdata, Vector3.zero, Quaternion.identity, base.gameObject, "Full", false);
            return;
        }
        SanctuaryManager.CreatePet(pdata, dragonMarker.position, dragonMarker.rotation, base.gameObject, "Full", false);
    }
    ...
    public void SetCurrentDragon(SanctuaryPet inDragon)
    {
        this.mCurrentDragon = inDragon;
        if (this.mCurrentDragon != null)
        {
            if (this._PetSkinMapping != null)
            {
    ...
    public SanctuaryPet mCurrentDragon;
    ...
    internal bool <GetTutorial>b__0(ScientificExperiment.LabTutorialData data)
    {
